...
const TopRevenueContributors: React.FC<Props> = ({ data }) => {
    ...
    return (
        <div className="premium-card p-6 h-full flex flex-col group hover-scale shadow-sm relative overflow-hidden">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Top Revenue Contributors</h3>
                <button className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider hover:text-indigo-700 transition-colors">View All Customers</button>
            </div>
                ...
                <table className="w-full text-[11px]">
                    <thead className="text-slate-400 uppercase tracking-wider font-bold border-b border-slate-100">
                        <tr>
                            ...
                            <th className="text-center py-3 px-2">Invoiced</th>
                            <th className="text-left py-3 px-2 w-1/3">Collection Progress</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {customers.map((customer, idx) => (
                            <tr key={idx} className="hover:bg-slate-50/50 transition-colors group/row">
                                <td className="py-4 px-2 font-bold text-slate-800">{customer.name}</td>
                                <td className="py-4 px-2 text-center text-slate-500 font-bold">{customer.invoiced}</td>
                                <td className="py-4 px-2">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <span className="text-[10px] font-black text-slate-400 group-hover/row:text-slate-600 transition-colors w-7 text-right">{customer.percent}%</span>
                                    </div>
                                </td>
                                <td className="py-4 px-2 text-right">
                                    <span className={`text-[9px] font-black border px-2 py-1 rounded-lg shadow-sm ${getStatusStyles(customer.status)}`}>
                                        {customer.status}
                                    </span>
                                </td>
                            </tr>
